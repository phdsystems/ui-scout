...
export function createMockPageWithDOM(htmlFixture: string) {
  ...
  const mockPage = {
    ...
    $$eval: vi.fn().mockImplementation((selector: string, fn: Function, ...args: any[]) => {
      const elements = document.querySelectorAll(selector);
...
function createMockElement(element: Element) {
  ...
  return {
    ...
    getAttribute: vi.fn().mockImplementation((attr: string) => 
      Promise.resolve(element.getAttribute(attr))
    ),
...
function createMockLocator(elements: NodeListOf<Element>) {
  ...
  return {
    ...
    nth: vi.fn().mockImplementation((index: number) => 
      createMockElement(elementArray[index])
    ),
    evaluate: vi.fn().mockImplementation((fn: Function) => {
      return Promise.resolve(elementArray.map(el => fn(el)));
    }),
    evaluateAll: vi.fn().mockImplementation((fn: Function) => {
      return Promise.resolve(elementArray.map(el => fn(el)));
    }),
  };
}
...
export function countElementsByType(mockPage: any, type: string): number {
  ...
  switch(type) {
    case 'button':
      ...
      break;
    case 'input':
      selector = 'input:not([type="button"]):not([type="submit"]):not([type="reset"]), textarea, select';
      break;
    case 'navigation':
